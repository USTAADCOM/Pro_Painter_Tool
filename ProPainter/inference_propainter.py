...
def make_final_video(folder_path):
    video_file_list = glob.glob(f"{folder_path}/*.mp4")
    loaded_video_list = []
    ...
    merged_video_name = f"{folder_path}/inpaint_out.mp4"
    final_clip.write_videofile(f"{merged_video_name}")
    
def make_final_masked_video(folder_path):
    video_file_list = glob.glob(f"{folder_path}/*.mp4")
    loaded_video_list = []
    for video in video_file_list:
        print(f"Adding video file:{video}")
        loaded_video_list.append(VideoFileClip(video))
    final_clip = concatenate_videoclips(loaded_video_list)
    merged_video_name = f"{folder_path}/masked_in.mp4"
    final_clip.write_videofile(f"{merged_video_name}")

if __name__ == '__main__':
  # Access cuda or cpu device
    # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
  #Access the default device on running environment
    device = get_device()
    output_sub_videos = []
    masked_sub_videos = []
    ...
    frames1, fps, size1, video_name = read_frame_from_videos(args.video)
    if not args.width == -1 and not args.height == -1:
        size1 = (args.width, args.height)
    if not args.resize_ratio == 1.0:
        size1 = (int(args.resize_ratio * size1[0]), int(args.resize_ratio * size1[1]))
    frames1, size1, out_size = resize_frames(frames1, size1)
    ...
    # masked video folder
    save_masked_root = os.path.join(args.output+"_masked", video_name)
    if not os.path.exists(save_masked_root):
        os.makedirs(save_masked_root, exist_ok=True)
    if args.mode == 'video_inpainting':
        frames_len = len(frames1)
        flow_masks1, masks_dilated1 = read_mask(args.mask, frames_len, size1,
        ...
        frames1, flow_masks1, masks_dilated1, size1 = extrapolation(frames1, (args.scale_h, args.scale_w))
        w, h = size1
    else:
    ...
    if len(frames1) < 100:
      ...
      for i in range(len(frames1)):
        mask_ = np.expand_dims(np.array(masks_dilated1[i]),2).repeat(3, axis=2)/255.
        img = np.array(frames1[i])
      ...
      imageio.mimwrite(os.path.join(save_masked_root, 'masked_in.mp4'), masked_frame_for_save, fps=fps, quality=7)
      torch.cuda.empty_cache()
    else:
      # More then 50 frames
      masked_chunk = np.ceil(len(frames1) / 100).astype(int)
      for chunk_num in range(masked_chunk):
          print("Chunk: ", chunk_num + 1, " of ", masked_chunk, " chunks")
          if chunk_num + 1 == masked_chunk:
            masked_frame_for_save = []
            masked_frames = frames1[chunk_num * 100 : ]
            for i in range(len(masked_frames)):
              ...
              fuse_img = mask_ * fuse_img + (1-mask_)*img
              masked_frame_for_save.append(fuse_img.astype(np.uint8))
            masked_sub_videos.append(os.path.join(save_masked_root, f'masked_in_{chunk_num}.mp4'))
            imageio.mimwrite(os.path.join(save_masked_root, f'masked_in_{chunk_num}.mp4'), masked_frame_for_save, fps=fps, quality=7)
            torch.cuda.empty_cache()
          else:
            masked_frame_for_save = []
            masked_frames = frames1[chunk_num * 100 : (chunk_num + 1 ) * 100]
            for i in range(len(masked_frames)):
              mask_ = np.expand_dims(np.array(masks_dilated1[i]),2).repeat(3, axis=2)/255.
              img = np.array(frames1[i])
              green = np.zeros([h, w, 3])
            ...
            imageio.mimwrite(os.path.join(save_masked_root, f'masked_in_{chunk_num}.mp4'), masked_frame_for_save, fps=fps, quality=7)
            torch.cuda.empty_cache()
      make_final_masked_video(save_masked_root)
      for file_path in masked_sub_videos:
        os.remove(file_path)
      print(masked_sub_videos)
    ##############################################
    ...
    ##############################################
    if frames_len < 50:
        # Less then 50 frames excution
        frames_inp = [np.array(f).astype(np.uint8) for f in frames1]
        frames = to_tensors()(frames1).unsqueeze(0) * 2 - 1
        flow_masks = to_tensors()(flow_masks1).unsqueeze(0)
        masks_dilated = to_tensors()(masks_dilated1).unsqueeze(0)
        ...
        imageio.mimwrite(os.path.join(save_root, f'inpaint_out.mp4'), comp_frames, fps=fps, quality=7)
        print(f'\nAll results are saved in {save_root}')
        torch.cuda.empty_cache()
    else:
        # More then 50 frames excution
        chunk = np.ceil(frames_len / 50).astype(int)
        for i in range(chunk):
          print("Chunk: ", i+1, " of ", chunk, " chunks")
          if i+1 == chunk:
                  frames = frames1[i*50:]
                  flow_masks = flow_masks1[i*50:]
                  masks_dilated = masks_dilated1[i*50:]
          else:
                  frames = frames1[i*50:(i+1)*50]
                  flow_masks = flow_masks1[i*50:(i+1)*50]
                  masks_dilated = masks_dilated1[i*50:(i+1)*50]
          frames_inp = [np.array(f).astype(np.uint8) for f in frames1]
          ...
          with torch.no_grad():
              ...
              for f in tqdm(range(0, video_length, neighbor_stride)):
                  ...
                  with torch.no_grad():
                      ...
                      binary_masks = masks_dilated[0, neighbor_ids, :, :, :].cpu().permute(
                          0, 2, 3, 1).numpy().astype(np.uint8)
                      for k in range(len(neighbor_ids)):
                          idx = neighbor_ids[k]
                          img = np.array(pred_img[k]).astype(np.uint8) * binary_masks[k] \
                              + ori_frames[idx] * (1 - binary_masks[k])
                          if comp_frames[idx] is None:
                              comp_frames[idx] = img
                          else:
                              comp_frames[idx] = comp_frames[idx].astype(np.float32) * 0.5 + img.astype(np.float32) * 0.5
              ...
              if args.save_frames:
                  for idx in range(video_length):
                      ...
                      imwrite(f, img_save_root)
              comp_frames = [cv2.resize(f, out_size) for f in comp_frames]
              output_sub_videos.append(os.path.join(save_root, f'inpaint_out_{i}.mp4'))
              imageio.mimwrite(os.path.join(save_root, f'inpaint_out_{i}.mp4'), comp_frames, fps=fps, quality=7)
              print(f'\nAll results are saved in {save_root}')
              torch.cuda.empty_cache()
    if frames_len > 50:
        make_final_video(save_root)
        for file_path in output_sub_videos:
          os.remove(file_path)
        print(output_sub_videos)